from common import http_client
from config.configuration import secrets, settings
from model.file import IngestionFlowFileType, FileOrigin


def post_upload_file(token, traceparent: str, organization_id: int, ingestion_flow_file_type: IngestionFlowFileType,
                     file_origin: FileOrigin, file_name: str):
    return http_client.post(
        url=f'{secrets.internal_base_url}{settings.api.ingress_path.fileshare}/organization/{organization_id}/ingestionflowfiles',
        headers={
            'Authorization': f'Bearer {token}',
            'traceparent': f'{traceparent}'
        },
        params={
            'ingestionFlowFileType': ingestion_flow_file_type.value,
            'fileOrigin': file_origin.value,
            'fileName': file_name
        },
        files={
            'ingestionFlowFile': (file_name, open(file_name, 'rb'))
        },
        timeout=settings.default_timeout
    )


def get_ingestion_flow_file(token, traceparent: str, organization_id: int, ingestion_flow_file_id: int):
    return http_client.get(
        url=f'{secrets.internal_base_url}{settings.api.ingress_path.fileshare}/organization/{organization_id}/ingestionflowfiles/{ingestion_flow_file_id}',
        headers={
            'Authorization': f'Bearer {token}',
            'traceparent': f'{traceparent}'
        },
        timeout=settings.default_timeout
    )


def get_export_file(token, traceparent: str, organization_id: int, export_file_id):
    return http_client.get(
        url=f'{secrets.internal_base_url}{settings.api.ingress_path.fileshare}/organization/{organization_id}/exportfiles/{export_file_id}',
        headers={
            'Authorization': f'Bearer {token}',
            'traceparent': f'{traceparent}'
        },
        timeout=settings.default_timeout
    )
