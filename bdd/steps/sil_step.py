...
def step_sil_create_spontaneous(context, soap_action):
    """Creates the mixed debt position through SIL (`paaSILInviaCarrello`) and asserts the SOAP outcome is `OK` with a URL to proceed for payment."""

    installment = context.installment
    org_info = context.org_info

    if soap_action == 'InviaCarrello':
        res = post_sil_invia_carrello_dovuti(token=context.token,
        ...
        expected_pattern = checkout_url_pattern(context.org_info.fiscal_code)
